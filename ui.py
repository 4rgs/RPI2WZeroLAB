...
font_mod   = ImageFont.truetype(FONT_PATH, 12)
...
def draw_top_bar(draw, info, image):
    ...
    bx = draw.textbbox((0, 0), hostname, font=font_small)
    x_host = (WIDTH - (bx[2] - bx[0])) // 2
    draw.text((x_host, 0), hostname, font=font_small, fill=0)
...
def draw_bt_info(draw, info):
    lines = [
        ...
        f"Name   : {info.get('bt_name', 'Ninguno')}",
...
def render_ui(epd, info, mode):
    ...
    try:
        if hasattr(epd, "displayPartial"):
            ...
            epd.display(epd.getbuffer(image))  # fallback
    except Exception as e:
        print(f"❌ Error al refrescar e-ink: {e}")
